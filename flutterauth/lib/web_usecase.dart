...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> loginAction() async {
    final url = Uri.https(AUTH0_DOMAIN, '',{
      'response_type': 'code',
    ...
    try {
      final result = await FlutterWebAuth.authenticate(url: url.toString(), callbackUrlScheme: appBaseUrl);
      ...
      final response = await http.post(Uri.parse('$AUTH0_DOMAIN/token'), body: {
        'client_id': AUTH_CLIENT_ID,
        'redirect_uri': appBaseUrl,
      ...
      // webStorage.setString("refresh_token", refreshToken);
